---
title: Introdução
sidebar_label: Introdução
sidebar_position: 1
description: Visão geral do desenvolvimento do design.
---

# Design

Seção dedicada a apresentação área de design do projeto.

A prototipação ocorrerá pelo figma: [Figma do projeto](https://www.figma.com/team_invite/redeem/osOk4my6ME4H5P0ujJF0Wf)

---

# Referências para criação das páginas

- [Prisma](https://estudeprisma.com/);
- [OUL Educação](https://educacao.uol.com.br/);
- [Khan acadamy](https://pt.khanacademy.org/);
- [brilliant](https://brilliant.org/);

---

# Requisitos

1. Atender a responsividade do maior número possível de dispositivos.
2. Usar componentes no figma para facilitar a reutilização
3. Padronização de componentes
4. Uso de auto layout Figma para auxiliar na responsividade
5. Uso de Constraints para auxiliar na responsividade
6. Usar o principio atomic design para criação de componentes, layouts, templates e pages

## Componentes

- Botões Gerais
- Botões de ação
- Footer
- Header
- Inputs
- Navbar

## Páginas do projeto

- [Index](./pages/indexPage.md) - Aqui deverá ficar a área de apresenção do projeto
- [Página de Conteúdo](./pages/conteudoPage.md)
- [Conteúdo por assunto](./pages/conteudoPage.md)
- [Simulados](./pages/simuladosPage.md)
- [Página do Simulado](./pages/simuladoPage.md)
- [Questões](./pages/questoes.md)
- [Questões por assunto](./pages/questoes.md)
- [Resumo dos professores](./pages/resumeProfessores.md)
- [Turma](./pages/turma.md)
- [Turma Professor](./pages/turma.md)
- [Turma aluno](./pages/turma.md)
- [Turma área de atividade](./pages/turma.md)
- [Turma participantes](./pages/turma.md)
- [Turma atividades](./pages/turma.md)
- [Turma provas](./pages/turma.md)
- [Turma simulados](./pages/turma.md)
- [Login](./pages/login.md)
- [Register](./pages/register.md)
